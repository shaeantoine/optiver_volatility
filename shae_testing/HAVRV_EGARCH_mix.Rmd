...
for (j in 1:4) {
  ...
  for (i in 1 : length(cluster_log_r1[[j]])) {
    vol.train[[i]] <- cluster_vol[[j]][[i]][1:10, ]
    vol.val[[i]] <- cluster_vol[[j]][[i]][-(1:10), ]
  }
...
for (j in 1:4) { 
  ...
  for (i in 1:length(cluster_log_r1[[j]])) {
    vol.train[[i]] <- cluster_vol[[j]][[i]][1:10, ]  # First 16 Time buckets for Training
    vol.val[[i]] <- cluster_vol[[j]][[i]][-(1:10), ]  # Remaining 4 Time buckets for validation
  }
...
for (i in 1: 10){
  ...
  mix_mean = mean(MSE.lm[is.finite(unlist(MSE.lm)) & !is.nan(unlist(MSE.lm))])
...
mix = vector("list", length = length(pred.lm))
for (i in 1:length(pred.lm)){
  mix[[i]] = cluster_pred_lm[[4]][[i]]*1 + cluster_rv_pred[[4]][[i]][6:10]*0
}
QLIKE.lm.mix <- vector()
for (i in 1 : length(vol)) {
...
mean(QLIKE.lm.mix[is.finite(unlist(QLIKE.lm.mix)) & !is.nan(unlist(QLIKE.lm.mix))])
mean(QLIKE.lm.garch[is.finite(unlist(QLIKE.lm.garch)) & !is.nan(unlist(QLIKE.lm.garch))])
