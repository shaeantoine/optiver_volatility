...
for (j in 1:4) {
  ...
  for (i in 1:length(cluster_vol.val[[j]])) {
    ...
    mean.vol <- sum(cluster_vol.val[[j]][[i]]$volatility[11:15])/5
    
    list_HAV1_cluster[[i]] <- data.frame(
      vol_1 = cluster_vol.val[[j]][[i]]$volatility[15],
      mean_vol_5 = mean.vol
    )
    ...
    for (t in 16:20) {
        ...
        mean.vol <- sum(cluster_vol.val[[j]][[i]]$volatility[(t-4):t])/5
        ...
        # Predict volatility for the current time period
        pred.lm[[t]] <- predict(list_HAV_wls_cluster[[j]][[i]], newdata = list_HAV1_cluster[[i]])
        
        # Update mean volatility for the next iteration
        mean.vol <- (mean.vol * 5 - cluster_vol.val[[j]][[i]]$volatility[t-4] + cluster_vol.val[[j]][[i]]$volatility[t+1])/5
    }
...
```{r}
# Function to predict volatility with feedback loop
predict_volatility_feedback <- function(data, wls_model, hist_length = 5, pred_length = 5) {
  # Extract volatility data
  volatility <- data$volatility

  # Initialize predictions list and mean volatility
  predictions <- list()
  mean_vol <- head(volatility, hist_length)
  
  # Loop through prediction steps
  for (i in (hist_length + 1):(hist_length + pred_length)) {
    # Prepare data for prediction
    new_data <- data.frame(
      vol_past = volatility[-(i - hist_length:i - 1)],
      mean_vol_past = mean_vol
    )
    
    # Predict volatility using WLS model
    prediction <- predict(wls_model, newdata = new_data)
    
    # Update mean volatility with past predictions
    mean_vol <- c(mean_vol[-1], predictions[[length(predictions)]])
    mean_vol <- mean_vol / length(mean_vol)
...
wls_model = list_HAV_wls_cluster[[j]][[i]]
hist_length = 5
pred_length = 5
...
for (i in (hist_length + 1):(hist_length + pred_length)) {
  # Prepare data for prediction
  new_data <- data.frame(
    vol_1 = volatility[(i - hist_length):(i - 1)],
    mean_vol_5 = mean_vol
  )
...
predictions <- predict_volatility_feedback(data = cluster_vol.val[[j]][[i]], wls_model = list_HAV_wls_cluster[[j]][[i]])

# Access individual predictions
first_prediction <- predictions[[hist_length + 1]]
last_prediction <- predictions[[hist_length + pred_length]]
```
...
for (j in 1:4) {
  ...
  for (i in 1:length(cluster_vol.val[[j]])) {
    ...
    if (i < length(cluster_vol.val[[j]])) {
      mean.vol <- (pred.lm[[i]] + mean.vol) / 2
    }
  }
...
# Initialize a variable to store all predictions
all_predictions <- c()
...
for (j in 1:4) {
  ...
  # Initialize mean_vol with the first 5 actual volatilities (assuming these are available)
  mean.vol <- head(cluster_vol.val[[j]][[1]]$volatility, 5)
  
  for (i in 1:length(cluster_vol.val[[j]])) {
    list_HAV1_cluster[[i]] <- data.frame(
      vol = cluster_vol.val[[j]][[i]]$volatility[-(1:5)],
      vol_1 = cluster_vol.val[[j]][[i]]$volatility[5:(len.val - 1)],
      # Include previous prediction or mean_vol in a new column
      mean_vol_5 = c(mean.vol[-1], mean.vol[i]) # previous prediction
    )
    
    pred.lm[[i]] <- predict(list_HAV_wls_cluster[[j]][[i]], newdata = list_HAV1_cluster[[i]])
    
    # Update mean.vol using ALL past predictions (including current)
    mean.vol <- (c(all_predictions, pred.lm[[i]])) / (i + 5)
    
    # Store all predictions for feedback loop
    all_predictions <- c(all_predictions, pred.lm[[i]])
  }
