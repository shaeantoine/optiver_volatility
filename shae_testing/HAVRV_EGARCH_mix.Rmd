...
for (i in 1: 10){
  ...
  #mix
  mix = vector("list", length = length(cluster_pred_lm[[1]]))
  for (i in 1:length(cluster_pred_lm[[1]])) {
    mix[[i]] = cluster_pred_lm[[1]][[i]]*weight_hav + cluster_rv_pred[[1]][[i]][6:10]*weight_garch
  }
  
  MSE.lm <- vector()
  QLIKE.lm <- vector()
  for (i in 1 : length(vol)) {
    MSE.lm <- c(MSE.lm, mean((cluster_vol_val[[j]][[i]]$volatility[6:10] - mix[[i]]) ^ 2))
    QLIKE.lm <- c(QLIKE.lm, mean(cluster_vol_val[[j]][[i]]$volatility[6:10] / mix[[i]] - 
                                   log(cluster_vol_val[[j]][[i]]$volatility[6:10] / mix[[i]]) - 1))
  }
...
mix = vector("list", length = length(pred.lm))
for (i in 1:length(pred.lm)){
  mix[[i]] = cluster_pred_lm[[1]][[i]]*0.5 + cluster_rv_pred[[1]][[i]][6:10]*0.5
}
QLIKE.lm.mix <- vector()
for (i in 1 : length(vol)) {
  QLIKE.lm.mix <- c(QLIKE.lm, mean(cluster_vol_val[[1]][[i]]$volatility[6:10] / mix[[i]] - 
                                 log(cluster_vol_val[[1]][[i]]$volatility[6:10] / mix[[i]]) - 1))
}
boxplot(x=QLIKE.lm.mix, horizontal = TRUE, main = 'QLIKE for Mixture on Cluster 1', ylim=c(0,2))


QLIKE.lm.garch <- vector()
for (i in 1 : length(vol)) {
  QLIKE.lm.garch <- c(QLIKE.lm, mean(cluster_vol_val[[1]][[i]]$volatility[6:10] / cluster_rv_pred[[1]][[i]][6:10] - 
                                 log(cluster_vol_val[[1]][[i]]$volatility[6:10] / cluster_rv_pred[[1]][[i]][6:10]) - 1))
}

boxplot(x=QLIKE.lm.garch, horizontal = TRUE, main = 'QLIKE for GARCH on Cluster 1', ylim=c(0,2))
```
