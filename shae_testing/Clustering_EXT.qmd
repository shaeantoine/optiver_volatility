...
#df_cl <- df_cl[c("name", "imbalance", "rush")]#, "mean_BAS")] 
...
plot = ggplot(df, aes(x = mean_WAP, y = imbalance, color = cluster, label = names)) +
  geom_point() +
  geom_text(aes(label=names), vjust = -1, hjust = 1) +
  theme_minimal() +
  labs(title = "Cluster Plot", x = "mean_WAP", y = "imbalance")

ggplotly(plot)

#plot_ly(x = df$imbalance, y = df$mean_WAP, type = "scatter3d", mode = "markers", marker = list(color = df$cluster))
