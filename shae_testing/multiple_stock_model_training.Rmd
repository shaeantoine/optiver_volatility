...
all_stocks <- list.files(dir_stocks) # CHANGE
five_stocks <- sample(all_stocks, 5)
# 50: 66 cluster 1
# 7: 84 cluster 2
# 126: 28 cluster 3
# 75: 89 cluster 4
...
all_stocks[28]
all_stocks[89]
four_stocks = c(all_stocks[66], all_stocks[84], all_stocks[28], all_stocks[89])
...
# Selecting the training data of the 500th 10 minute interval of the 4th stock
stock_wlr_train[[4]][[500]]
...
# WLR Model Evaluation on Multiple stocks

```{r}

# # Single stock reference
# list.reg.val <- list()
...
# stock = stock_files_list[[i]]
# vol.train[[1]] = stock_train_val[[j]]$train[[1]]
# vol.val[[1]] = stock_train_val[[j]]$val[[1]]
# list.reg[[i]] = stock_wlr_train[[j]][[i]]
# ##########


# Multiple stock implementation 
stock_vol_pred <- list()
for (j in 1 : length(stock_files_list)) {
  list.reg.val <- list()
  len.val <- length(stock_train_val[[j]]$val[[1]]$volatility)
  pred.lm <- list()
  ...
  for (i in 1 : length(stock_volatility_list[[j]])) {
    ...
    list.reg.val[[i]] <- data.frame(volatility = stock_train_val[[j]]$val[[i]]$volatility, 
                                    price = mean.price$WAP[len.train:(len.train + len.val - 1)],
                                    order = mean.order$num_order[len.train:(len.train + len.val - 1)],
                                    BidAskSpread = mean.BAS$BidAskSpread[len.train:(len.train + len.val - 1)])
    
    pred.lm[[i]] <- predict(wlr_model_list[[j]][[i]], newdata = list.reg.val[[i]])
  }
  
  stock_vol_pred[[j]] <- pred.lm
  
}

# This is the volatility prediction of the last 4 30 second intervals of the 400th 10 minute bucket for the first stock
stock_vol_pred[[1]][[400]]
```
...
for (j in 1 : length((stock_files_list))) {
  MSE.lm <- vector()
  for (i in 1 : length(stock_volatility_list[[j]])) {
    MSE.lm <- c(MSE.lm, mean((stock_train_val[[j]]$val[[i]]$volatility - stock_vol_pred[[j]][[i]]) ^ 2))
  }
  MSE_all[[j]] <- MSE.lm
}

boxplot(MSE_all, horizontal = TRUE, main = "MSE for all stocks", names = paste("Stock", 1:length(stock_files_list)), ylim = c(0,0.000015))
...
for (j in 1 : length((stock_files_list))) {
  QLIKE.lm <- vector()
  for (i in 1 : length(stock_volatility_list[[j]])) {
    QLIKE.lm <- c(QLIKE.lm, mean(stock_train_val[[j]]$val[[i]]$volatility / stock_vol_pred[[j]][[i]] - 
                                   log(stock_train_val[[j]]$val[[i]]$volatility / stock_vol_pred[[j]][[i]]) - 1))
  }
  QLIKE_all[[j]] <- QLIKE.lm
  #boxplot(QLIKE.lm, horizontal = TRUE, main = paste("Stock", j, "QLIKE"))
}

boxplot(QLIKE_all, horizontal = TRUE, main = "QLIKE for all stocks", names = paste("Stock", 1:length(stock_files_list)), ylim = c(0,5))
