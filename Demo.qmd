    ...
    labs(
      title = paste("Prediction Result\ncluster ",as.character(cluster_l[[i]]),
                    if(cluster_l[[i]] == 2|cluster_l[[i]] == 4){mod = "HAV + WLR"}
                       else {mod = "EGARCH + WLR,"}," weight for each time interval : "),
      tag = as.character(i),
      subtitle = paste( weight_p),
...
agg_mix$mod_1 <- rep(NaN,nrow(agg_mix))
agg_mix$mod_2 <- rep(NaN,nrow(agg_mix))


for(i in 1:length(vol)){
  if(cluster_l[[i]]==1){
    agg_mix$c1[(((i-1)*20) +1):(i*20)] <- agg_mix$val[(((i-1)*20) +1):(i*20)]
    agg_mix$mod_1[(((i-1)*20) +1):(i*20)] <- agg_mix$pred_f[(((i-1)*20) +1):(i*20)]
    }
  if(cluster_l[[i]]==2){
    agg_mix$c2[(((i-1)*20) +1):(i*20)] <- agg_mix$val[(((i-1)*20) +1):(i*20)]
    agg_mix$mod_2[(((i-1)*20) +1):(i*20)] <- agg_mix$pred_f[(((i-1)*20) +1):(i*20)]
    }
  if(cluster_l[[i]]==3){
...
agg_plot <- ggplot(agg_mix, aes(x=time)) +
  ...
  geom_line(aes(y = mod_1,color = "EGARCH + WLR"), linetype="twodash")+
  geom_line(aes(y = mod_2,color = "HAV+ WLR"), linetype="twodash")+
